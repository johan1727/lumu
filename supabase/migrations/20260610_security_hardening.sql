...
-- IMPORTANTE: hay que revocar de PUBLIC — anon/authenticated heredan EXECUTE de PUBLIC,
-- así que revocar solo de esos roles no cierra el acceso vía /rest/v1/rpc/
REVOKE EXECUTE ON FUNCTION public.handle_new_user() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.detect_premium_changes() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.upsert_query_intent_memory(text, text, text, text, text, text, text) FROM PUBLIC, anon, authenticated;
